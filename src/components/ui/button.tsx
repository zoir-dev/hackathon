    ...
    "inline-flex items-center justify-center whitespace-nowrap rounded-md text-sm font-medium transition-colors focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring disabled:pointer-events-none disabled:opacity-50 gap-3",
    {
        variants: {
            variant: {
                ...
                outline: "border border-primary text-primary bg-background shadow-sm hover:bg-accent hover:text-accent-foreground",
...
const Button = React.forwardRef<HTMLButtonElement, ButtonProps>(
    ({ className, variant, size, asChild = false, loading, icon, ...props }, ref) => {
        ...
        return (
            ...
            >
                {loading && (
                    <Spinner color={variant === "destructive" ? "secondary" : "primary-foreground"} size="sm" />
                )}{" "}
